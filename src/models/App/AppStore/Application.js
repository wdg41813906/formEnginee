...
import { ApplicationsGetListPaged ,New,Remove} from '../../../services/App/Application'
...
export default {
  ...
  effects: {
    *Copy({ payload }, { call, put }) {
      ...
      if (data.isValid) {
        ...
        yield put({
          ...
          payload: {
            pageIndex:payload.pageIndex,pageSize:config.pageSize
          }
        })
    ...
    *ApplicationsGetListPaged({ payload }, { call, put }) {
      ...
      message.loading("加载中...")
      const { data } = yield call(ApplicationsGetListPaged, parse(payload));
      if (data) {
        message.destroy()
        yield put({
          type: 'AppGetSuccess',
